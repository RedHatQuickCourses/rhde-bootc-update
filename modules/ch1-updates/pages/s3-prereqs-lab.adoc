...
Most activities in this course will focus on the _test machine_.
...
If you are reusing the same classroom environment of the https://redhatquickcourses.github.io/rhde-bootc-build/[previous^] https://redhatquickcourses.github.io/rhde-bootc-provision/[courses^] in this series, you should have everything ready and can skip most of thee steps.
...
--
IMPORTANT: The `podman search` command, and similar commands from other tools such as Docker, are neither reliable to list container images in a production container registry nor to verify the availability of an image by its name and tag.
...
5. If necessary, clone the course samples repository from GitHub.
...
.. If you think that your clone may be outdated, because you started the previous courses before this course was released, you can just update your local clone.
+
[source,subs="verbatim,quotes"]
--
...
.. Invoke the script that runs the bootc image builder utility.
+
...
.. Invoke the script that creates a _test VM_ from the ISO.
+
After the installation is complete and the VM reboots, detach from its virtual serial console by using kbd:[Ctrl+C].
+
[source,subs="verbatim,quotes"]
--
...
.. Use the SSH key to access your _test VM_.
...
Later in this course, we will also demonstrate how to update an image mode system to a new bootc container image from removable media, for scenarios in which network access to an OCI container registry is not practical.
